...
//tinserach를 찾은거를 바탕으로 node를 삭제한다. 제시된 값을 가지는 노드를 삭제. 중복된 경우 먼저 나온걸 삭제
//단, Child Node가 있는 경우 최대한 Child Node를 보존하는 쪽으로 하였다.
void tdel(tree node)
{
	tree temp = node;
	tree tm, ltm;
	
	if(node->lt == 1 && node->rt == 1)//RCH LCH 없을때 parent에게 적절한 Link를 준다.
	{
	...
	else//node에 LCH RCH있으면
	{
		//node의 Lch의 Rch맨 끝 부분까지 간다.
		...
		while(1)
		{
			if(ltm->rt == 1)
			{
				...
				ltm->lc = node->lc;
				ltm->lt = node->lt;
				ltm->parent = node->parent;//해당 node가 대체한다.
				
				if(node == node->parent->rc)//parent node와 연결.
				{
					node->parent->rc = ltm;
				}
				else if(node == node->parent->lc)
		...
		while(1)//오른쪽편의 맨끝 LCH가 수정된 node를 가리키도록.
		{
			if(ltm->rt == 1)
			{
				break;
...
tree copy(tree orig)
{
	...
	if(orig)
	{
		...
		temp->lt = orig->lt;
		temp->rt = orig->rt;
		orig->copied = temp;//Thread형식으로 가리킬 것을 대비하여 복사된 것의 좌표를 저장한다.
		...
		if(orig->rc != orig)//root의 rc에서의 무한반복 방지.
		{
			if(temp->rt == 1)//Thread면
			{
				temp->rc = orig->rc->copied;//원본에 있는 복사된 주소 활용
			}
			else if(temp -> rt == 0)//아니면 원래대로 간다.
			{
				temp->rc = copy(orig->rc);
			}
		}
		else//root부분에 대한 예외처리...
		{
			temp->rc = temp;
		}
...
int main()
{
	...
	else if (tmp == '-')//삭제 -> 1-4에서 구현. 제시된 값을 가지는 노드를 삭제.
	{
		fscanf(fin, "%d", &pos);
