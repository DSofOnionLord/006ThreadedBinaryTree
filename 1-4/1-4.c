...
{
	tree temp = (root)->rc;
	//printf("%d %d %d %d %d\n", root->val,root->rt,root->lt, root->rc->val, root->lc->val);
	if((root)->rt == 0)
	{
		while(temp->lt == 0)
		{
...
//tinserach를 찾은거를 바탕으로 node를 삭제한다. 제시된 값에 해당하는 노드 삭제, 중복시 먼저 나오는 노드삭제
...
int main()
{
	...
	else if (tmp == '-')//삭제 -> 1-4에서 구현. 제시된 값에 해당하는 노드를 삭제합니다.
	{
		fscanf(fin, "%d", &pos);
